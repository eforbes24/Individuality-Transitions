...
class World:
    def __init__(self, ID, timesteps, X, Y, start_CA, CAfood, clusters, cdev, 
        ...
        CAFinds, y = make_blobs(n_samples=self.CAfood, n_features=2, centers=self.centers, center_box=self.center_box, cluster_std=self.center_dev)
    ...
    def checkDeath(self):
        for cell in self.CAs:
            if cell.fill <= 0:
                cell.end = self.steps
                self.data.append(["cell", "died_uni", cell.ID, cell.start, cell.end, 
                                  cell.lifespan, cell.inmulti, cell.children])
                posx = cell.x
                posy = cell.y
            ...
            for cell in multi.CAs:
                if cell.fill <= 0:
                    ...
                    for cell in multi.CAs:
                        ...
                        self.data.append(["cell", "died_multi", cell.ID, cell.start, cell.end, 
                                  cell.lifespan, cell.inmulti, cell.children])
    ...
    def step(self):
        ...
        for m in self.Multis:
            nCA = nCA + len(m.CAs)
            for c in m.CAs:
                c.inmulti = c.inmulti + 1
        nCA = nCA + len(self.CAs)
        self.run.append([self.steps,nUni,nMulti,nCA,self.CAfood])
